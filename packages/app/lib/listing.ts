...
type Snapshot = NonNullable<
  Awaited<ReturnType<typeof orpc.contentIndex.latest>>
>;
...
export const revisionOf = (version: string) =>
  Number(version.split(".").pop()) || 0;
...
export function supportsGameVersion(listing: Listing, version: string) {
  ...
  return listing.releases.some(
    (release) =>
      ...
      (release.game_max_revision == null ||
        revision <= release.game_max_revision),
  );
}
