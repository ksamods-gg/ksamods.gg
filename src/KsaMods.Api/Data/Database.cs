...
public sealed class Database(NpgsqlDataSource source)
{
    ...
    /// contains one. Converting here means the deployment can paste in whatever its provider gave
    /// it.</para>
    /// </summary>
    public static string Normalise(string connectionString)
    {
        ...
        foreach (var pair in uri.Query.TrimStart('?').Split('&', StringSplitOptions.RemoveEmptyEntries))
        {
            ...
            {
                // Worth naming: libpq accepts some keywords Npgsql spells differently, and the
                // raw exception says only that a key was not found.
        ...
        // Only when the URL is silent. A shared database is easy to exhaust, and the previous
        // compose file set this explicitly - dropping to Npgsql's default on the way to a URL
        // would be an invisible change of behaviour.
